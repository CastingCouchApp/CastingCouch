...
describe("Alerts library page", () => {
        ...
        invokeMock.mockImplementation(
            async (cmd: string, args?: Record<string, unknown>) => {
                ...
                if (cmd === "alert_runtime") {
                    ...
                    if (typeof args?.obsSceneName === "string") {
                        runtime = {
                            ...runtime,
                            obs_scene_name: args.obsSceneName,
                        };
                    }
                    return runtime;
                }
    ...
    it("renders library rows and runtime status", async () => {
        ...
        expect(
            screen.getByRole("checkbox", { name: "Alerts aktiv" }),
        ).toBeChecked();
    });
    ...
    it("toggles a disabled alert via upsert_alert", async () => {
        ...
        await user.click(
            within(cheerRow as HTMLElement).getByRole("button", {
                name: "Aktivieren",
            }),
        );
        await waitFor(() =>
    ...
    it("deletes after confirmation", async () => {
        ...
        await user.click(
            within(followRow as HTMLElement).getByRole("button", {
                name: "Löschen",
            }),
        );
        await waitFor(() =>
            expect(invokeMock).toHaveBeenCalledWith("delete_alert", {
                alertType: "Follow",
            }),
        );
    });
    ...
    it("saves OBS scene name", async () => {
        ...
        await user.click(
            screen.getByRole("button", { name: "Szene speichern" }),
        );
        await waitFor(() =>
            expect(invokeMock).toHaveBeenCalledWith(
                "alert_runtime",
                expect.objectContaining({ obsSceneName: "overlay" }),
            ),
        );
    });
});

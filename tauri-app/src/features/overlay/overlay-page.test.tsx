...
describe("Overlay canvas page", () => {
    ...
    it("shows overlay health when fetch succeeds", async () => {
        renderOverlay();
        expect(
            await screen.findByText("Overlay-Server: erreichbar"),
        ).toBeInTheDocument();
    ...
    it("opens editor via open_overlay_editor with editor URL", async () => {
        renderOverlay();
        fireEvent.click(
            await screen.findByRole("button", { name: "Editor öffnen" }),
        );
        await waitFor(() =>
    ...
    it("copies the view URL", async () => {
        renderOverlay();
        fireEvent.click(
            await screen.findByRole("button", { name: "URL kopieren" }),
        );
        await waitFor(() =>
    ...
    it("duplicates a canvas", async () => {
        renderOverlay();
        fireEvent.click(
            await screen.findByRole("button", { name: "Duplizieren" }),
        );
        await waitFor(() =>
            expect(invokeMock).toHaveBeenCalledWith("duplicate_canvas", {
                id: "default",
            }),
        );
    });

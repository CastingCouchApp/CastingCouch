...
function SettingsForm({
    ...
    const [original, setOriginal] = useState(() => cloneSettings(initial));
    const [obsPassword, setObsPassword] = useState("");
    const form = useForm({
        defaultValues: cloneSettings(original),
        onSubmit: async ({ value }) => {
            const next = applyEditedSettings(original, value);
            applyThemeId(next.General.ThemeId);
            try {
                await onSave(next, original, obsPassword);
                setOriginal(cloneSettings(next));
                setObsPassword("");
            } catch {
                /* Mutation error is rendered above the form. */
            }
        },
    });
            ...
            <Card className="space-y-3">
                ...
                <label className="block text-sm">
                    ...
                    <Input
                        ...
                        autoComplete="off"
                        placeholder={
                            obsHasPassword ? "Passwort hinterlegt" : ""
                        }
                        value={obsPassword}

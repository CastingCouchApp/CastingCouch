...
function OverlayPage() {
    ...
    const create = useMutation({
        mutationFn: (name: string) =>
            tauriInvoke<CanvasDto>("create_canvas", { name }),
    ...
    const duplicate = useMutation({
        mutationFn: (id: string) =>
            tauriInvoke<CanvasDto>("duplicate_canvas", { id }),
        onSuccess: invalidate,
    });
    const update = useMutation({
        mutationFn: (args: { id: string; name?: string; selected?: boolean }) =>
            tauriInvoke("update_canvas", args),
        onSuccess: invalidate,
    });
    ...
    const columns = useMemo<LegacyColumnDef<CanvasDto>[]>(
                ...
                cell: ({ row }) => {
                    ...
                    return (
                        <div className="flex flex-wrap gap-2">
                            <Button
                                variant="ghost"
                                onClick={() => openEditor.mutate(canvas)}
                            >
                                Editor öffnen
                            </Button>
                            <Button
                            ...
                            <Button
                                variant="ghost"
                                onClick={() => duplicate.mutate(canvas.id)}
                            >
                                Duplizieren
                            </Button>
                            <Button
        ...
        [canDelete, duplicate, openEditor, remove, update],
    );

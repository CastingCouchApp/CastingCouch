...
public sealed class ObsBrowserSourceInstaller
{
    ...
    public ObsBrowserSourceInstaller(
        ISettingsStore settingsStore,
        IObsWebSocketClient obs,
        IOverlayDataService overlay,
        IAppLogger logger)
    {
        _settingsStore=settingsStore; _obs=obs; _overlay=overlay; _logger=logger;
    }
    ...
    public async Task<IReadOnlyList<string>> InstallAsync(CancellationToken ct=default)
    {
        ...
        var root=await _overlay.GetOverlayRootAsync(ct);
        ...
        foreach(var d in definitions.Where(x=>x.Enabled))
        {
            if(!File.Exists(d.File)) throw new FileNotFoundException("Overlay-Datei fehlt.",d.File);
    ...
    {
        if(!_obs.IsConnected) throw new InvalidOperationException("OBS ist nicht verbunden.");
        if(string.IsNullOrWhiteSpace(scene)) throw new InvalidOperationException("Bitte eine OBS-Szene auswählen.");
        var settings=await _settingsStore.LoadAsync(ct);
        var root=await _overlay.GetOverlayRootAsync(ct);
        var safe=(contentType ?? "content-name").Trim().ToLowerInvariant();
        ...
        if(!allowed.Contains(safe)) throw new InvalidOperationException("Unbekanntes Content-Modul.");
        var file=Path.Combine(root,"modules",safe+".html");
        var source="ccs_"+safe.Replace('-','_');
    ...
    public async Task<string> InstallGoalAsync(string goalType, CancellationToken ct=default)
    {
        ...
        var settings=await _settingsStore.LoadAsync(ct);
        var root=await _overlay.GetOverlayRootAsync(ct);
        var normalized=(goalType ?? "").Trim().ToLowerInvariant();
        ...
        var source="ccs_"+normalized+"_goal";
        var goalScene = string.IsNullOrWhiteSpace(settings.Obs.GoalOverlayScene)
    ...
    public async Task<string> InstallAllGoalsAsync(CancellationToken ct=default)
    {
        ...
        {
            if(!File.Exists(definition.File)) throw new FileNotFoundException("Overlay-Datei fehlt.",definition.File);

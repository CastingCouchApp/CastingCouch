...
public sealed class WorkflowE2eService : IWorkflowE2eService
{
    private readonly WorkflowModule _workflow; private readonly IFeatureGate _gate;
    public WorkflowE2eService(WorkflowModule workflow, IFeatureGate gate) { _workflow=workflow; _gate=gate; }
    public async Task<WorkflowE2eReport> RunAsync(CancellationToken ct=default)
    {
        await _gate.RequireAsync(FeatureCatalog.Workflow, ct); var started=DateTimeOffset.Now; var steps=new List<WorkflowE2eStepResult>();
